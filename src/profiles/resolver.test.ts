...
const TEST_CONFIG_DIR = join(process.env['TEMP'] ?? '/tmp', 'gwcli-resolver-test-' + Date.now());
...
describe('resolveProfileName', () => {
  beforeEach(() => {
    ...
    delete process.env['GWCLI_PROFILE'];
  });
  ...
  it('Priority 1 — returns flag profile; flag wins over GWCLI_PROFILE env var', () => {
    process.env['GWCLI_PROFILE'] = 'env-profile';
    expect(resolveProfileName('flag-profile')).toBe('flag-profile');
  });

  it('Priority 2 — returns GWCLI_PROFILE env var when no flag is given', () => {
    process.env['GWCLI_PROFILE'] = 'env-profile';
  ...
  it('Priority 4 — throws GwcliError with code NO_PROFILE when nothing is configured', () => {
  ...
  it('throws GwcliError with code INVALID_PROFILE_NAME for a traversal name supplied via flag', () => {
    let thrownErr: unknown;
    try {
