...
//
// mgws ships a default Desktop-type OAuth client so `mgws profiles add <name>`
...
//
// The defaults are overridable at runtime via `MGWS_CLIENT_ID` /
// `MGWS_CLIENT_SECRET` so the shipped client can be rotated (or replaced with
// an org-specific one) without a rebuild.
export const DEFAULT_OAUTH_CLIENT_ID =
  process.env['MGWS_CLIENT_ID'] ??
  '446192415694-qii4n0omi544khef2760l1v560m167lm.apps.googleusercontent.com';

export const DEFAULT_OAUTH_CLIENT_SECRET =
  process.env['MGWS_CLIENT_SECRET'] ?? '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';
