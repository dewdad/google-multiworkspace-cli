# src — gwcli TypeScript source

## Purpose

TypeScript source for `gwcli`, the multi-profile orchestration layer over the external `gws` CLI. `index.ts` is the binary entrypoint (`dist/index.js`). gwcli never calls Google APIs directly — it resolves a profile and spawns `gws` with that profile's credentials.
...
- Directly owned single-purpose modules (no child docs — too small to warrant a boundary):
  ...
  - `types/index.ts` — shared types + `GwcliError`.
  ...
  - `version.ts` — `GWCLI_VERSION` string.
...
- **User-facing errors** throw `GwcliError(message, code, suggestion?)`. Top-level handlers (in `index.ts` and each command action) print `message` + optional `suggestion` to stderr and `process.exit(1)`. Do not `console.log` raw errors.
...
- **Version sync.** `version.ts` `GWCLI_VERSION` must match `package.json` `version` and `skill/SKILL.md` `metadata.version`. Bump all three together.
