...
/**
 * Preflight exit codes (gwcli-namespace, range 60-69).
 *
 ...
 *
 * Note: gwcli itself is implicitly verified because this command IS gwcli.
...
export async function runPreflight(options: PreflightOptions = {}): Promise<void> {
  ...
  try {
    ...
    if (err instanceof GwcliError) {
      emit({ ok: false, error: 'gws_missing', code: err.code, fix: 'gwcli setup' });
