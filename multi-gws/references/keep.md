...
> **⚠ Most users cannot use this API.** Google's official Keep API is Workspace-only and requires an admin to enable the Keep API in the Google Cloud project. Personal `@gmail.com` accounts — which are likely most `mgws` users — will get `403 PERMISSION_DENIED` or `insufficient_scope` on every command in this reference. `mgws` does not work around this; the error from `gws` is the signal that the API is not available for that account.
>
...
There is no consumer Keep support in `gws`, and therefore none in `mgws`. There is no roadmap to add it — Google has not exposed a public API for consumer Keep, and `mgws` does not call Google APIs directly.
...
- **Master-token auth, not OAuth.** Authentication uses a token captured from a Google Android sign-in flow — a separate trust decision from `mgws`'s OAuth profiles. The token grants broad account access; treat it as a credential, not a session.
- **Independent of `mgws`.** No shared credentials, no shared profiles, no integration. These tools are mentioned here only because the official-API path is not available to consumer accounts.
...
**Agents:** do **not** install, configure, or invoke KeepSidian or KIM on the user's behalf. If a user asks for consumer Keep access, surface the links above with the caveats, then stop. These tools live outside `mgws`'s trust and support boundary.
...
```bash
mgws [--profile <name>] keep <resource> <method> --params '<json>' [--json '<request-body>']
```
...
mgws keep notes list --params '{"pageSize":50,"filter":"trashed=false"}'
```
...
### List Labels
```bash
mgws keep notes list --params '{"filter":"label=MyLabel"}'
```
...
```bash
mgws keep notes permissions list --params '{"parent":"notes/<note-id>"}'
```

### Share a Note
```bash
mgws keep notes permissions create --params '{"parent":"notes/<note-id>"}' --json '{
...
### "Show my notes"
```bash
mgws keep notes list --params '{"pageSize":20}'
```

### "Create a quick note"
```bash
mgws keep notes create --json '{"title":"Quick Note","body":{"text":{"text":"Content here"}}}'
```
...
```bash
mgws keep notes list --params '{"filter":"trashed=false","pageSize":50}'
```
...
```bash
mgws keep notes list --params '{"pageSize":25,"pageToken":"<token>"}'
```
